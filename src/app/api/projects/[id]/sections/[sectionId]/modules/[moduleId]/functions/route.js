...
export async function PUT(request, { params }) {
  try {
    ...
    if (!Array.isArray(functions)) {
      return NextResponse.json({ error: "Functions must be an array" }, { status: 400 });
    }
    ...
    const functionData = functions.map(func => ({
      title: func.title.trim(),
      createdAt: new Date(),
      updatedAt: new Date(),
      descriptions: []
    }));
    ...
    project.srsDocument.sections[sectionIndex].modules[moduleIndex].functions = functionData;
    project.srsDocument.sections[sectionIndex].modules[moduleIndex].updatedAt = new Date();
    ...
    return NextResponse.json({
      ...
      functions: project.srsDocument.sections[sectionIndex].modules[moduleIndex].functions,
    });
  } catch (error) {
